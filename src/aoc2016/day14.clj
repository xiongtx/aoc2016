...
(defn hash-stream
  ([salt]
   (hash-stream salt 0))
  ([salt start]
   (map #(md5 (str salt %)) (drop start (range)))))

(defn pad-key?
  [hash stream salt i]
  (if-let [[[c]] (re-find #"(.)\1{2}" hash)]
    (boolean (some #(re-find (re-pattern (format "(%s)\\1{4}" c)) %)
                   (take 1000 (drop (inc i) stream))))
    false))

(defn pad-key-stream
  ([salt]
   (pad-key-stream salt (hash-stream salt)))
  ([salt stream]
   (->> stream
        (map-indexed #(vector %1 %2))
        (filter (fn [[i hash]]
                  (pad-key? hash stream salt i))))))
...
(defn stretched-hash-stream
  ([salt]
   (stretched-hash-stream salt 0))
  ([salt start]
   (pmap #(md5-times 2016 %) (hash-stream salt start))))

(defn index-64-stretched-key
  [salt]
  (->> salt
       stretched-hash-stream
       (pad-key-stream salt)
       (take 64)
       last
       first))
