...
;; set of identical spherical capsules that are carried to the top and allowed
...
(defn falls-through?
  [state wait]
  (->> state
       (map-indexed (fn [i [disk npos pos]]
                      (mod (+ pos (inc i) wait) npos)))
       (every? zero?)))

(defn min-wait
  [state]
  (let [[disk max-npos pos] (->> state
                                 (sort-by #(nth % 1) >)
                                 first)]
    (loop [wait (mod (- max-npos pos disk) max-npos)]
      (if (falls-through? state wait)
        wait
        (recur (+ wait max-npos))))))
...
(defn add-new-disk
  [state]
  (conj state [(inc (count state)) 11 0]))
